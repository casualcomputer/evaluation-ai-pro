...
def context_chat(prompt: str, query_engine: RetrieverQueryEngine):
    try:
        ...
        with open('utils/prompt_template.txt', 'r') as file:
            prompt_template = file.read()
        
        # Replace placeholder with actual query
        prompt_with_context = prompt_template.replace("{{ query }}", prompt)

        # Use the prompt with context in the query engine
        stream = query_engine.query(prompt_with_context)
