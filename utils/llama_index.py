...
def read_documents(directory_path: str):
    ...
    Notes:
        The `directory_path` parameter should be a path to a directory containing files that represent the documents to be loaded. The function will iterate over all files in the directory, and load their contents into a list of strings.
    """
    try:
        loader = SimpleDirectoryReader(input_dir=directory_path, recursive=True)
